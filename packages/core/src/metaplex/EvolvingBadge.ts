 ...
 * 
 * This module handles badge evolution based on on-chain progress:
 * - XP accumulation updates badge level
 * - Mission completions evolve badges
 * - Streak bonuses upgrade rarity
 */
...
export function calculateRarity(level: number): EvolvingBadgeRarity {
    ...
    if (level >= LEVEL_THRESHOLDS.SILVER) return 'SILVER';
    return 'BRONZE';
}
...
/**
 * EvolvingBadge - Handles on-chain badge evolution
 * 
 * This is the key feature for the Metaplex Track:
 * "NFTs that evolve based on player actions"
 */
export class EvolvingBadge {
    private umi: any;
    ...
    /**
     * Get current badge state from blockchain
     */
    async getBadgeState(badgeAddress: string): Promise<BadgeState | null> {
        try {
            ...
            console.error('[EvolvingBadge] Failed to fetch state:', error);
            return null;
        }
    }
    ...
    async evolveBadge(
        ...
        const attributeList = [
            ...
            { key: 'Missions', value: (currentState.missionsCompleted + 1).toString() },
        ];
        ...
        try {
            ...
            const newState: BadgeState = {
                ...
                level: newLevel,
                rarity: newRarity,
                missionsCompleted: currentState.missionsCompleted + 1,
                lastUpdated: new Date(),
            };
    ...
    /**
     * Batch evolve - for users with multiple badges
     */
    async evolveMultipleBadges(
...
export function previewEvolution(currentXp: number, additionalXp: number) {
    ...
    return {
        ...
        newLevel,
        currentRarity: calculateRarity(calculateLevel(currentXp)),
        newRarity,
        willLevelUp: newLevel > calculateLevel(currentXp),
        willRarityUpgrade: newRarity !== calculateRarity(calculateLevel(currentXp)),
    };
}

export default EvolvingBadge;
